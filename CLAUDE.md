...
- **Never use `href="#…"` for in-page navigation.** `App.tsx` reads the hash to choose the topic, so an anchor navigates the reader off the lesson. Use a button and `scrollIntoView`. The exceptions are links that really do mean to leave: the home grid's cards and the cross-topic `#sets` link in `NumTowerCard`.
