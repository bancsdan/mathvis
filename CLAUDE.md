...
| `src/components/<Name>Page.tsx` | One per lesson. `SetsPage.tsx` is the worked example of a long one; `LogicPage.tsx` is the second lesson and reuses the sets kit (same 12-element universe, `VennDiagram` with `elements`); `CombiPage.tsx` is the third and adds `ChoiceTree`/`GraphDiagram`; `NumPage.tsx` is the fourth and adds `NumberLine`; `PowPage.tsx` is the fifth and needs no new diagram component — its pictures are `.pow-factor` chips and one small SVG; `AlgPage.tsx` is the sixth and reuses the pow chips and `LineChart`, its area pictures being small per-card SVGs; `PropPage.tsx` is the seventh, on everyday finance and units — small `LineChart` multiples, an SVG percent bar and an invoice table; `LinPage.tsx` is the eighth — a balance SVG, step-through solvers, `LineChart` for the graphical methods and `NumberLine` for inequalities; `QuadPage.tsx` is the ninth — the parallel derivation table, `LineChart` parabolas with a gap-aware series for inequalities, `NumberLine` solution sets; `FnPage.tsx` is the tenth and adds `ArrowDiagram` — plus a dot plot of its own, `LineChart` with a dashed base curve under a transformed one, and an inverse mirrored in y = x. |
| `src/lib/` | Pure logic, no React, unit tested. `sets.ts` and `venn.ts` are the pattern; `logic.ts` builds on `sets.ts` (`Predicate`, `UNIVERSE`); `combinatorics.ts` holds the counting, choice trees, timetable constraints, graphs and the number sieve; `numbers.ts` holds set membership, long division, decimal ↔ fraction, nested intervals, interval notation, estimation and string-based rounding; `powers.ts` holds integer, zero and negative exponents, the laws of powers as factor groups, string-based normal form and arithmetic in it, square roots and n-th roots; `algebra.ts` holds terms and polynomials with one tex writer, like terms, the linear tracker of the number tricks, monomial arithmetic, the named identities and completing the square; `proportion.ts` holds direct and inverse proportion, the graph shapes, the unit ladders, percent, chained percentage changes, the household bill and compound interest; `linear.ts` holds sides and fractions, the balance moves, solution sets over a base set, the inequality solver with its sign flip, both elimination methods for a two-by-two system, and the word-problem models; `quadratic.ts` holds the standard form, factoring, completing the square, the formula with its parallel derivation, the inequality solver, the substitutions and the word-problem models; `functions.ts` holds the assignment kinds with the node that spoils them, rules on a finite domain with table, range and preimages, the zeros, extremes and monotone runs of a polyline, the three elementary graphs with the solution counts of f(x) = c, the transformation steps, the linear inverse and the two practical models. |
...
Shared helpers: `Tex.tsx` (`<Tex tex="A \cup B" block />`), `useWidth.ts` (ResizeObserver, returns `[ref, width]`), `LineChart.tsx` (function plotting only; pass `format` when the y values are money, or the axis reads `1.0e+5`, and `xStep` when x counts whole things, or years get half-year ticks; a non-finite value leaves a gap, so a series can be drawn only where a condition holds). Lesson kit, reuse rather than rebuild: `VennDiagram.tsx`, `Exercise.tsx`, `Definition.tsx`, `SetsElementGrid.tsx`, `ChoiceTree.tsx`, `GraphDiagram.tsx`, `NumberLine.tsx`, `ArrowDiagram.tsx`, `useActiveSection.ts`.
